...
- **Multi-client routing by tool name** — During playtest, both the plugin client and playtest bridge client are registered with the Rust server. `enqueue_tool_request` routes by tool name: playtest-only tools (virtualuser, npc_driver, playtest_stop, logs) go to the bridge client (identified by `plugin_version` containing "playtest"); plugin tools (test_script, run_script, checkpoint, playtest_play/run) go to the main plugin. Falls back to most recently polled client if preferred type unavailable. Tool handlers in the plugin for playtest-only features should be stubs that return clear errors as a safety net.
